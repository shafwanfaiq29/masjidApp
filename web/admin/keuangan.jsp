...
boolean canAccess = isAdmin || isRiayah;
...
double filteredPemasukan = 0, filteredPengeluaran = 0;
if (hasFilter) {
    Connection conFilter = null;
    try {
        conFilter = Koneksi.getKoneksi();
        if (conFilter != null) {
            PreparedStatement ps = conFilter.prepareStatement("SELECT kategori, SUM(jumlah) as total FROM keuangan WHERE tanggal >= ? AND tanggal <= ? GROUP BY kategori");
            ps.setString(1, startDate);
            ps.setString(2, endDate);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                if ("Pemasukan".equalsIgnoreCase(rs.getString("kategori"))) {
                    filteredPemasukan = rs.getDouble("total");
                } else {
                    filteredPengeluaran = rs.getDouble("total");
                }
            }
            rs.close();
            ps.close();
        ...
        if (conFilter != null) try { conFilter.close(); } catch (Exception e) {}
    }
}
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .summary-card { background: white; border-radius: 16px; padding: 22px; display: flex; flex-direction: column; gap: 12px; box-shadow: 0 8px 25px rgba(0, 0, 0, 0.05); position: relative; overflow: hidden; }
        ...
        .summary-card .icon-wrapper { width: 50px; height: 50px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 22px; }
        ...
        .summary-card h4 { color: #888; font-size: 0.85rem; font-weight: 500; }
        .summary-card .amount { color: #333; font-size: 1.3rem; font-weight: 700; }
        ...
        .btn-info { background: linear-gradient(135deg, #2196f3, #64b5f6); color: white; }
        .form-actions { margin-top: 25px; display: flex; gap: 15px; flex-wrap: wrap; }
        .filter-form { display: flex; gap: 15px; align-items: flex-end; flex-wrap: wrap; margin-bottom: 25px; padding: 20px; background: linear-gradient(135deg, #e3f2fd, #bbdefb); border-radius: 12px; }
        .filter-form .form-group { margin-bottom: 0; }
        .filter-results { background: linear-gradient(135deg, #e8f5e9, #c8e6c9); border-radius: 12px; padding: 20px; margin-bottom: 20px; }
        .filter-results h4 { color: #2e7d32; margin-bottom: 15px; display: flex; align-items: center; gap: 8px; }
        .filter-results-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; }
        .filter-result-item { background: white; padding: 15px; border-radius: 10px; text-align: center; }
        .filter-result-item .label { font-size: 0.8rem; color: #666; margin-bottom: 5px; }
        .filter-result-item .value { font-size: 1rem; font-weight: 700; }
        .filter-result-item .value.pemasukan { color: #4caf50; }
        .filter-result-item .value.pengeluaran { color: #f44336; }
        .filter-result-item .value.saldo { color: #ff9800; }
        ...
        .badge.pemasukan { background: linear-gradient(135deg, #e8f5e9, #c8e6c9); color: #2e7d32; }
        .badge.pengeluaran { background: linear-gradient(135deg, #ffebee, #ffcdd2); color: #c62828; }
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="card">
            <h3><i class="fas fa-filter"></i> Filter Rentang Waktu</h3>
            <form method="get" class="filter-form">
                ...
                <button type="submit" class="btn btn-info"><i class="fas fa-search"></i> Tampilkan</button>
                <% if (hasFilter) { %>
                <a href="keuangan.jsp" class="btn btn-secondary"><i class="fas fa-times"></i> Reset</a>
                <% } %>
            </form>
            <% if (hasFilter) { %>
            <div class="filter-results">
                <h4><i class="fas fa-chart-pie"></i> Hasil Filter: <%= startDate %> s/d <%= endDate %></h4>
                <div class="filter-results-grid">
                    <div class="filter-result-item">
                        <div class="label">Pemasukan</div>
                        <div class="value pemasukan"><%= formatRupiah.format(filteredPemasukan) %></div>
                    </div>
                    <div class="filter-result-item">
                        <div class="label">Pengeluaran</div>
                        <div class="value pengeluaran"><%= formatRupiah.format(filteredPengeluaran) %></div>
                    </div>
                    <div class="filter-result-item">
                        <div class="label">Selisih</div>
                        <div class="value saldo"><%= formatRupiah.format(filteredSaldo) %></div>
                    </div>
                </div>
            </div>
            <% } %>
...
try {
    ...
    if (con != null) {
        ...
        if (hasFilter) {
            sql += " WHERE tanggal >= '" + startDate + "' AND tanggal <= '" + endDate + "'";
        }
...
%>
                        <tr>
                            <td><%= tgl %></td>
                            <td><%= ket %></td>
                            <td><span class="badge <%= kat.toLowerCase() %>"><i class="fas <%= "Pemasukan".equalsIgnoreCase(kat) ? "fa-arrow-up" : "fa-arrow-down" %>"></i> <%= kat %></span></td>
                            <td><strong><%= formatRupiah.format(jml) %></strong></td>
                            <td>
                                <button class="btn-edit" onclick="openEdit(<%= id %>, '<%= tgl %>', '<%= ketJs %>', '<%= kat %>', <%= jml %>)"><i class="fas fa-pen-to-square"></i> Edit</button>
...
if (!hasData) {
%>
                        <tr><td colspan="5" class="no-data"><i class="fas fa-wallet"></i>Belum ada data transaksi</td></tr>
<%
}
%>
                    </tbody>
