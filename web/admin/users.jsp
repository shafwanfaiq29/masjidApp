...
<%
if(session.getAttribute("adminId")==null){response.sendRedirect("../login.jsp");return;}
String adminRole=(String)session.getAttribute("adminRole");
if(adminRole==null) adminRole="Admin";
boolean isAdmin=adminRole.contains("Admin");
if(!isAdmin){response.sendRedirect("dashboard.jsp?error=access");return;}
boolean canAccessKegiatan=isAdmin||adminRole.contains("Imarah");
boolean canAccessKeuangan=isAdmin||adminRole.contains("Riayah");
boolean canAccessArsip=isAdmin||adminRole.contains("Idarah");
...
<title>Kelola User - Admin Masjid</title>
...
.checkbox-group label { display: flex; align-items: center; gap: 8px; cursor: pointer; padding: 10px 16px; border: 2px solid #e8e8e8; border-radius: 10px; transition: all 0.3s }
.checkbox-group label:hover { border-color: #1a5d3a; background: #f8fff8 }
.checkbox-group input[type="checkbox"] { width: 18px; height: 18px; accent-color: #1a5d3a }
.checkbox-group input[type="checkbox"]:checked+span { color: #1a5d3a; font-weight: 600 }
...
.badge { display: inline-flex; align-items: center; gap: 6px; padding: 6px 14px; border-radius: 20px; font-size: 0.82rem; font-weight: 500; margin: 2px }
.badge.admin { background: linear-gradient(135deg, #e3f2fd, #bbdefb); color: #1565c0 }
.badge.idarah { background: linear-gradient(135deg, #e8f5e9, #c8e6c9); color: #2e7d32 }
.badge.imarah { background: linear-gradient(135deg, #f3e5f5, #e1bee7); color: #7b1fa2 }
...
.modal-content { background: white; border-radius: 20px; padding: 35px; width: 90%; max-width: 480px; max-height: 90vh; overflow-y: auto }
...
@media(max-width:992px) {
  ...
  .sidebar-header h2, .sidebar-header p, .nav-menu a span, .role-badge { display: none }
  .sidebar-header .logo { font-size: 35px }
...
<div class="sidebar">
  <div class="sidebar-header">
    ...
    <p>Admin Panel</p><span class="role-badge"><i class="fas fa-user-tag"></i> <%=adminRole%></span>
  </div>
  <ul class="nav-menu">
    ...
    <%}%>
    <%if(canAccessUsers){%>
    <li><a href="users.jsp" class="active"><i class="fas fa-users-cog"></i><span>Kelola User</span></a></li>
    <%}%>
    <li class="logout"><a href="../LogoutServlet"><i class="fas fa-right-from-bracket"></i><span>Logout</span></a></li>
  </ul>
</div>
<div class="main-content">
  <div class="header"><i class="fas fa-users-cog"></i>
    <h1>Kelola User</h1>
  </div>
  ...
  <div class="alert alert-success"><i class="fas fa-check-circle"></i> User berhasil diperbarui!</div>
  ...
  <div class="alert alert-error"><i class="fas fa-exclamation-circle"></i> Tidak dapat menghapus akun sendiri!</div>
  ...
  <div class="card">
    ...
    <form action="../UserServlet" method="post"><input type="hidden" name="action" value="add">
      <div class="form-grid">
        <div class="form-group"><label><i class="fas fa-user"></i> Nama</label><input type="text" name="nama" required placeholder="Nama lengkap"></div>
        <div class="form-group"><label><i class="fas fa-at"></i> Username</label><input type="text" name="username" required placeholder="Username login"></div>
        <div class="form-group"><label><i class="fas fa-lock"></i> Password</label><input type="password" name="password" required placeholder="Password"></div>
        <div class="form-group"><label><i class="fas fa-user-tag"></i> Role </label>
          <div class="checkbox-group">
            <label><input type="checkbox" name="roles" value="Admin"><span>Admin</span></label>
            <label><input type="checkbox" name="roles" value="Idarah"><span>Idarah (Administrasi & Manajemen)</span></label>
            <label><input type="checkbox" name="roles" value="Imarah"><span>Imarah (Penyelenggaraan Kegiatan Ibadah)</span></label>
            <label><input type="checkbox" name="roles" value="Riayah"><span>Riayah (Penyelenggaraan Kegiatan Ibadah)</span></label>
          </div>
        </div>
      </div>
      <div class="form-actions"><button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Simpan</button><button type="reset" class="btn btn-secondary"><i class="fas fa-rotate-left"></i> Reset</button></div>
    </form>
  </div>
  <div class="card">
...
ResultSet rs=st.executeQuery("SELECT * FROM admin ORDER BY id ASC");
while(rs.next()){
hasData=true;
int id=rs.getInt("id");
...
boolean isSelf=(id==currentAdminId);
String deleteAttrs=isSelf?"disabled title='Tidak dapat menghapus akun sendiri'":"";
String namaJs=nama.replace("'","\\'").replace("\"","\\\"");
String[] roleList=role.split(",");
%>
          <tr>
            <td><%=no++%></td>
            <td><strong><%=nama%></strong><%=isSelf?" (Anda)":""%></td>
            <td><%=username%></td>
            <td>
              <%for(String r:roleList){String rb=r.trim().toLowerCase();%>
              <span class="badge <%=rb%>"><i class="fas fa-user-tag"></i><%=r.trim()%></span>
              <%}%>
            </td>
            <td>
              <button class="btn-edit" onclick="openEdit(<%=id%>,'<%=namaJs%>','<%=username%>','<%=role%>')"><i class="fas fa-pen-to-square"></i> Edit</button>
              <form action="../UserServlet" method="post" style="display:inline;" onsubmit="event.preventDefault(); confirmDeleteUser(this);">
                <input type="hidden" name="action" value="delete">
                <input type="hidden" name="id" value="<%=id%>">
                <button type="submit" class="btn-delete" <%=deleteAttrs%>><i class="fas fa-trash-can"></i> Hapus</button>
              </form>
            </td>
          </tr>
<%
...
%>
          <tr>
            <td colspan="5" class="no-data"><i class="fas fa-exclamation-triangle"></i>Error: <%=e.getMessage()%></td>
          </tr>
<%
} finally {
...
%>
          <tr>
            <td colspan="5" class="no-data"><i class="fas fa-users"></i>Belum ada user</td>
          </tr>
<%}%>
        </tbody>
      </table>
...
<div class="modal" id="editModal">
  <div class="modal-content">
    <div class="modal-header">
      <h3><i class="fas fa-pen-to-square"></i> Edit User</h3><button class="modal-close" onclick="closeEdit()"><i class="fas fa-times"></i></button>
    </div>
    <form action="../UserServlet" method="post"><input type="hidden" name="action" value="edit"><input type="hidden" name="id" id="e_id">
      <div class="form-group"><label><i class="fas fa-user"></i> Nama</label><input type="text" name="nama" id="e_nama" required></div>
      <div class="form-group"><label><i class="fas fa-at"></i> Username</label><input type="text" name="username" id="e_username" required></div>
      <div class="form-group"><label><i class="fas fa-lock"></i> Password Baru (kosongkan jika tidak diubah)</label><input type="password" name="password" placeholder="Biarkan kosong jika tidak diubah"></div>
      <div class="form-group"><label><i class="fas fa-user-tag"></i> Role (pilih satu atau lebih)</label>
        <div class="checkbox-group" id="e_roles">
          <label><input type="checkbox" name="roles" value="Admin" id="e_role_admin"><span>Admin</span></label>
          <label><input type="checkbox" name="roles" value="Idarah" id="e_role_idarah"><span>Idarah (Administrasi & Manajemen)</span></label>
          <label><input type="checkbox" name="roles" value="Imarah" id="e_role_imarah"><span>Imarah (Penyelenggaraan Kegiatan Ibadah)</span></label>
          <label><input type="checkbox" name="roles" value="Riayah" id="e_role_riayah"><span>Riayah (Pemeliharaan Fisik)</span></label>
        </div>
      </div>
      <div class="form-actions"><button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Simpan</button><button type="button" class="btn btn-secondary" onclick="closeEdit()"><i class="fas fa-times"></i> Batal</button></div>
    </form>
  </div>
</div>
<script>
function openEdit(id, nama, username, role) { document.getElementById('e_id').value = id; document.getElementById('e_nama').value = nama; document.getElementById('e_username').value = username; document.getElementById('e_role_admin').checked = role.includes('Admin'); document.getElementById('e_role_idarah').checked = role.includes('Idarah'); document.getElementById('e_role_imarah').checked = role.includes('Imarah'); document.getElementById('e_role_riayah').checked = role.includes('Riayah'); document.getElementById('editModal').classList.add('show'); }
function closeEdit() { document.getElementById('editModal').classList.remove('show'); }
document.getElementById('editModal').addEventListener('click', function (e) { if (e.target === this) closeEdit(); });
function confirmDeleteUser(form) { Swal.fire({ title: 'Apakah Anda yakin?', text: "User yang dihapus tidak dapat dikembalikan!", icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'Ya, hapus!', cancelButtonText: 'Batal' }).then((result) => { if (result.isConfirmed) { Swal.fire({ title: 'Menghapus...', didOpen: () => Swal.showLoading() }); form.submit(); } }); }
</script>
</body>
</html>
